...
const logStream = fs.createWriteStream('./coin-debug.log', { flags: 'a' })
let seq = 0

function logLine(label, data) {
  seq += 1
  const ts = new Date().toISOString()
  const ms = process.hrtime.bigint() / 1000000n // précision ms monotone
  logStream.write(`#${seq} [${ts}] (${ms}ms) ${label} ${JSON.stringify(data)}\n`)
}
...
// Tous les cycles de poll, filtrés utile seulement
Hopper.on('POLL_RAW', result => {
  // On ne garde que : les échecs, ou les polls qui contiennent un info (succès avec event)
  if (!result.success || result.info) {
    logLine('POLL_RAW', result)
  }
})

// Les events qui vous intéressent spécifiquement pour ce test
;['COIN_CREDIT', 'COIN_MECH_ERROR', 'COIN_MECH_JAMMED', 'COIN_MECH_RETURN_PRESSED'].forEach(evt => {
  Hopper.on(evt, info => logLine(evt, info))
})
...
/**
 * Lit la valeur totale disponible (toutes dénominations confondues) dans le Hopper,
 * via GET_ALL_LEVELS. Sert à détecter à l'avance un "Not enough value in device"
 * (errorCode 1) plutôt que de tenter un PAYOUT_AMOUNT voué à l'échec.
 * @returns {Promise<number|null>} valeur totale en centimes, ou null si indéterminable
 */
async function getAvailableHopperValue() {
  try {
    const levels = await Hopper.command('GET_ALL_LEVELS');
    const counter = levels && levels.info && levels.info.counter;
    if (!counter || typeof counter !== 'object') {
      console.warn('⚠️ Format inattendu pour GET_ALL_LEVELS (Hopper):', levels);
      return null;
    }
    let totalCents = 0;
    for (const key of Object.keys(counter)) {
      const entry = counter[key];
      if (entry && typeof entry.value === 'number' && typeof entry.denomination_level === 'number') {
        totalCents += entry.value * entry.denomination_level;
      }
    }
    return totalCents;
  } catch (err) {
    console.error('⚠️ Impossible de lire le stock de pièces Hopper:', err.message);
    logger.error(`Impossible de lire le stock de pièces Hopper: ${err.message}`);
    return null;
  }
}
 
/**
 * Lit le nombre réel de billets d'une dénomination donnée encore disponibles
 * dans le NV11 (recycleur/float), via GET_ALL_LEVELS.
 * @param {number} denominationCents - ex. 1000 pour 10€
 * @returns {Promise<number|null>} nombre de billets dispo, ou null si indéterminable
 * (erreur/format inattendu) — dans ce cas l'appelant doit décider du comportement
 * par défaut plutôt que de bloquer le rendu.
 */
async function getAvailableNoteCount(denominationCents) {
  try {
    const levels = await NV11.command('GET_ALL_LEVELS');
    const counter = levels && levels.info && levels.info.counter;
    if (!counter || typeof counter !== 'object') {
      console.warn('⚠️ Format inattendu pour GET_ALL_LEVELS (NV11):', levels);
      return null;
    }
    let total = 0;
    for (const key of Object.keys(counter)) {
      const entry = counter[key];
      if (entry && entry.value === denominationCents) {
        total += entry.denomination_level || 0;
      }
    }
    return total;
  } catch (err) {
    console.error('⚠️ Impossible de lire le stock de billets NV11:', err.message);
    logger.error(`Impossible de lire le stock de billets NV11: ${err.message}`);
    return null;
  }
}
...
async function handleRenduMixte(rendu) {
  ...
  console.log(`💶 Rendu demandé ${rendu}€ -> ${billets10}x10€ + ${reste}€ en pièces`);
  logger.info(`Rendu demandé ${rendu}€ -> ${billets10}x10€ + ${reste}€ en pièces`);
 
  // CORRECTIF : plafonner au stock réel de billets 10€ dans le NV11 et convertir
  // le manque en pièces, plutôt que de tenter des PAYOUT_NOTE voués à l'échec
  // (avec le risque de laisser une partie des billets déjà sortis physiquement
  // comptée comme un échec global).
  if (billets10 > 0) {
    const stockBillets = await getAvailableNoteCount(1000); // 1000 cts = 10€
 
    if (stockBillets === null) {
      console.warn('⚠️ Stock de billets 10€ indéterminable, on tente quand même le rendu demandé.');
      logger.error('Stock de billets 10€ indéterminable avant rendu — tentative sans plafonnement');
    } else if (stockBillets < billets10) {
      const manque = billets10 - stockBillets;
      const montantManquant = manque * 10;
 
      console.warn(`⚠️ Stock insuffisant: ${stockBillets} billet(s) de 10€ dispo pour ${billets10} demandé(s). ${montantManquant}€ convertis en pièces.`);
      logger.error(`Stock billets 10€ insuffisant: ${stockBillets}/${billets10} dispo, ${montantManquant}€ convertis en pièces`);
 
      postWithRetry({
        status: {
          transaction: transactionId,
          message: `Stock de billets 10€ insuffisant (${stockBillets}/${billets10} dispo) — ${montantManquant}€ rendus en pièces à la place`,
          value: 'warning'
        }
      }, SERVER_URL).catch(() => {});
 
      billets10 = stockBillets;
      reste = +(reste + montantManquant).toFixed(2);
    }
  }
 
  console.log(`💶 Rendu effectif: ${billets10}x10€ + ${reste}€ en pièces`);
  logger.info(`Rendu effectif: ${billets10}x10€ + ${reste}€ en pièces`);
  ...
  // === 2️⃣ Rendu pièces ===
  if (reste > 0) {
    let hopperAmount = Math.round(reste * 100);
 
    // CORRECTIF : PAYOUT_AMOUNT peut échouer avec errorCode 1 "Not enough value in
    // device" si le Hopper n'a pas assez de pièces en stock — erreur DÉFINITIVE
    // (pas de retry possible, contrairement au "busy" du NV11). Contrairement au cas
    // billets->pièces, il n'y a pas d'unité plus petite vers laquelle basculer : on
    // plafonne donc la demande au stock réel et on remonte le manque en avertissement
    // explicite (à traiter manuellement), plutôt que de tenter un rendu voué à l'échec.
    const stockHopper = await getAvailableHopperValue(); // en centimes
 
    if (stockHopper === null) {
      console.warn('⚠️ Stock de pièces indéterminable, on tente quand même le rendu demandé.');
      logger.error('Stock de pièces Hopper indéterminable avant rendu — tentative sans plafonnement');
    } else if (stockHopper < hopperAmount) {
      const manqueCts = hopperAmount - stockHopper;
 
      console.warn(`⚠️ Stock de pièces insuffisant: ${(stockHopper / 100).toFixed(2)}€ dispo pour ${(hopperAmount / 100).toFixed(2)}€ demandé(s). ${(manqueCts / 100).toFixed(2)}€ ne pourront pas être rendus automatiquement.`);
      logger.error(`Stock pièces insuffisant: ${stockHopper}/${hopperAmount} cts dispo, manque ${manqueCts} cts`);
 
      postWithRetry({
        status: {
          transaction: transactionId,
          message: `Stock de pièces insuffisant (${(stockHopper / 100).toFixed(2)}€/${(hopperAmount / 100).toFixed(2)}€ dispo) — ${(manqueCts / 100).toFixed(2)}€ restants à rendre manuellement`,
          value: 'warning'
        }
      }, SERVER_URL).catch(() => {});
 
      hopperAmount = stockHopper; // on ne demande que ce qui est réellement disponible
    }
 
    if (hopperAmount <= 0) {
      // Plus aucune pièce disponible : on ne tente même pas PAYOUT_AMOUNT (qui
      // échouerait de toute façon), mais on marque bien la tâche comme échouée
      // pour que le bilan final de handleRenduMixte le reflète.
      console.warn('⚠️ Aucune pièce disponible, rendu pièces annulé.');
      logger.error('Rendu pièces annulé: stock de pièces à 0');
      tasks.push(Promise.reject(new Error('Rendu pièces annulé: stock de pièces insuffisant (0€ disponible)')));
    } else {
      const resteEffectif = +(hopperAmount / 100).toFixed(2);
      console.log(`🪙 Hopper : rendu ${resteEffectif}€ (${hopperAmount} cts) en pièces...`);
      logger.info(`Hopper : rendu ${resteEffectif}€ (${hopperAmount} cts) en pièces...`);
 
      // Déclarés ici (hors de la Promise) pour pouvoir les détacher si PAYOUT_AMOUNT
      // échoue avant même qu'un DISPENSED/ERROR n'ait de chance d'arriver.
      let onDispensed;
      let onError;
 
      const dispensePromise = new Promise((resolve, reject) => {
        onDispensed = async (data) => {
          ...
          try {
            await postWithRetry(
              { status: { transaction: transactionId, note: resteEffectif, value: 'debited' } },
              SERVER_URL
            );
      ...
      tasks.push(
        Hopper.command('PAYOUT_AMOUNT', {
          ...
          .then((result) => {
            // CORRECTIF : comme pour PAYOUT_NOTE, PAYOUT_AMOUNT peut se RÉSOUDRE avec
            // success: false (refus métier — fonds insuffisants, mécanisme busy, etc.)
            // au lieu de rejeter. Sans cette vérification, on enchaînait vers dispensePromise
            // et on attendait pour rien un DISPENSED qui ne viendrait jamais.
            if (result && result.success === false) {
              const errorDesc = (result.info && result.info.error) || result.status;
              throw new Error(`PAYOUT_AMOUNT refusée par le Hopper: ${errorDesc}`);
            }
            return dispensePromise;
          })
          .catch((err) => {
            // Plus aucun DISPENSED/ERROR n'est attendu pour ce rendu pièces : on détache
            // les listeners pour éviter qu'ils ne traînent indéfiniment sur le Hopper.
            Hopper.off('DISPENSED', onDispensed);
            Hopper.off('ERROR', onError);
 
            console.error('❌ Échec du rendu pièces (PAYOUT_AMOUNT):', err.message);
            logger.error(`Échec du rendu pièces (PAYOUT_AMOUNT): ${err.message}`);
 
            // Avertissement demandé : notifier le serveur central en cas d'échec.
            postWithRetry({
              status: {
                transaction: transactionId,
                message: `Échec du rendu pièces (${err.message})`,
                value: 'warning'
              }
            }, SERVER_URL).catch((postErr) => {
              console.error(`⚠️ Échec de l'envoi de l'avertissement PAYOUT_AMOUNT: ${postErr.message}`);
            });
 
            throw err; // repropage pour que Promise.allSettled compte bien ce task en "rejected"
          })
      );
    }
  }
