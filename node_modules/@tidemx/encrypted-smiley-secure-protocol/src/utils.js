...
function parseData(data, currentCommand, protocolVersion, deviceUnitType) {
 /* console.log('🔧 parseData appelé - Command:', currentCommand);
  console.log('📊 Données brutes reçues:', Buffer.from(data).toString('hex'));
  console.log('📏 Longueur:', data.length, 'bytes');
*/
  const result = {
    success: data[0] === 0xf0,
    status: statusDesc[data[0]] !== undefined ? statusDesc[data[0]].name : 'UNDEFINED',
    command: currentCommand,
    info: {}, // ← ICI: initialisé comme objet
  }

  // === DÉTECTION ULTIME ===
  if (currentCommand === 'POLL' && data.length >= 10) {
    //console.log('🎯 ANALYSE POLL - Recherche pattern f0bf...');
    
    const dataHex = Buffer.from(data).toString('hex');
    //console.log('🔍 Données en hex:', dataHex);
    
    // === VERSION CORRIGÉE ET OPTIMISÉE ===
if (currentCommand === 'POLL' && data.length >= 10) {
  const dataHex = Buffer.from(data).toString('hex');
  
  // Recherche du pattern RM5 HD
  if (dataHex.includes('f0bf01')) {
    const patternIndex = dataHex.indexOf('f0bf01');
    const byteIndex = patternIndex / 2;
    
    // Vérifier qu'on a assez de données
    if (data.length >= byteIndex + 10) {
      const valueByte = data[byteIndex + 3];  // Le 4ème byte après f0bf01
      const countryCode = data.slice(byteIndex + 7, byteIndex + 10);
      ...
      const valueCents = RM5_VALUES[valueByte];
      ...
      if (valueCents !== undefined) {
        ...
        result.info.push({
          code: 223,
          name: 'COIN_CREDIT',
          description: `Coin credit: ${(valueCents / 100).toFixed(2)}€`,
          value: [{ 
            value: valueCents, 
            country_code: Buffer.from(countryCode).toString(),
            currency: 'EUR'
          }],
          raw_value: '0x' + valueByte.toString(16).padStart(2, '0'),
          raw_format: 'RM5_HD'
        });
        
        console.log(`💰 Pièce de ${(valueCents / 100).toFixed(2)}€ détectée (0x${valueByte.toString(16)})`);
        return result;
      } else {
        console.log(`❓ Valeur inconnue: 0x${valueByte.toString(16)} (${valueByte} décimal)`);
      }
    }
  }
}
    ...
    if (currentCommand === 'REQUEST_KEY_EXCHANGE') {
      ...
      if (data[0] !== undefined && statusDesc[data[0]] !== undefined) {
        ...
        // Dans parseData, section POLL
          ...
          } // Dans la section POLL de parseData, AVANT le cas COIN_CREDIT existant
          else if (info.name === 'COIN_CREDIT') {
            // Vérifier si c'est le format RM5 HD (f0bf en tête)
            if (chunk.length >= 8 && chunk[1] === 0xF0 && chunk[2] === 0xBF) {
              console.log('🔧 Format RM5 HD détecté pour COIN_CREDIT');
              
              const channel = chunk[3];
              const valueByte = chunk[4];
              const countryCode = Buffer.from(chunk.slice(8, 11)).toString();
              
              // Mapping des valeurs RM5 HD
              const RM5_VALUES = {
                0x01: 1, 0x02: 2, 0x05: 5, 0x0a: 10, 
                0x14: 20, 0x32: 50, 0x64: 100, 0xc8: 200
              };
              
              const valueCents = RM5_VALUES[valueByte] || 0;
              ...
              info.raw_format = 'RM5_HD';
              
              k += 11; // Taille du format RM5 HD
            }
            // Format standard (existant)
