...
module.exports = class SSP extends EventEmitter {
  ...
  /*poll(status = true) {
    if (status) {
      ...
      return this.exec('POLL').then(result => {
        if (result.info) {
          ...
          res.forEach(info => { this.emit(info.name, info) })
        } else if (result.error) {
          //logger.error('⚠️ POLL sans info:', result.error, result)
          console.log('⚠️ POLL sans info:', result.error, result)   // <-- ajout temporaire
        }
    ...
    if (status) {
      this.polling = true
      return this.exec('POLL').then(result => {
        this.emit('POLL_RAW', result)   // <-- ajout : TOUJOURS émis, succès ou pas
