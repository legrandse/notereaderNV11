...
function parseData(data, currentCommand, protocolVersion, deviceUnitType) {
  ...
  if (result.success) {
    ...
    while (k < data.length) {
      ...
      // === NOUVEAU : Détection pièces RM5 HD via pattern f0bf ===
      if (el === 0xF0 && data[k + 1] === 0xBF && (k + 9) < data.length) {
        console.log('🎯 Pattern f0bf détecté - Création événement COIN_CREDIT');
        
        const info = {
          code: 223, // 0xDF
          name: 'COIN_CREDIT', 
          description: 'Coin credit event from ccTalk device'
        };
        
        const channel = data[k + 2];
        const valueByte = data[k + 3];
        const countryCode = Buffer.from(data.slice(k + 7, k + 10)).toString();
