...
NV11.on('NOTE_HELD_IN_BEZEL', result => {
        const data = { status: { message: 'Veuillez svp récupérer le billet...', value: 'process' } };
        //noteInProcessing = false;
        postWithRetry(data, SERVER_URL)
            .catch(error => {
                console.error(`Erreur lors de l'envoi: ${data.status.message}`);
...
app.post('/hopperStack', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    // --- Récupération des nouveaux niveaux ---
    const levels = await Hopper.command('GET_ALL_LEVELS');
    console.log('📊 Tous les niveaux mis à jour:', levels);

    // --- Envoi au serveur central (Log / Synchro) ---
    // Le await ici garantit que l'envoi est tenté avant de répondre au client.
    // Le .catch() local empêche un crash si le serveur externe est down.
    await postWithRetry({
      status: {
        message: `Stored levels: ${JSON.stringify(levels.info.counter)}`,
        value: 'info'
